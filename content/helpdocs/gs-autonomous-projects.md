...
> **Scheduler:** the `{{brand.cli.binName}} autodrain` daemon, which you run on the same machine as `{{brand.cli.binName}} workers dev`. The daemon ticks every ~2 minutes and POSTs `/api/cli/autodrain/tick` with your registered CLI token; the platform fans out per-project decisions and the daemon dispatches the chosen sprint locally. Replaced the old GitHub Actions cron — that path was retired 2026-05-04 because platform-side scheduling for tenant tasks contradicts the auth model.
>
...
Notifications are deduped to **once per hour per project** so you get one ping per state change, not one per daemon tick (every ~2 min).

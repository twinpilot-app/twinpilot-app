...
> ⚠️ **Cloud mode is currently suspended.** The worker container image is no longer auto-published; primary path is **Local Mode** above. The deploy command still works for environments that have a previously-published image, but new deployments require manually re-running `publish-worker.yml` first. See [Cloud Orchestration](/help/prereq-cloud) for status.

...
| **Kanban (autonomous)** | Kanban backlog items | Dispatcher picks the first todo each tick | `{{brand.cli.binName}} autodrain` daemon (~2min ticks) within constraints |
...
- The `{{brand.cli.binName}} autodrain` daemon ticks ~every 2 minutes, asks the platform for the next decision per project, and (when allowed) atomically locks the chosen `todo` item (`status='doing'`, `sprint_id=…`) and dispatches one sprint per tick.
...
Check in order: pause flag, approval gate, daily cap reached, cooldown, active window, unproductive guard, on-empty policy. Skip reasons surface in the `{{brand.cli.binName}} autodrain --logs` output (or wherever you redirected the daemon's stdout) and in the project's last-tick badge in Office.

...
> ⚠️ **Cloud mode is currently suspended.** New operators should follow [Local Orchestration](/help/prereq-local). The cloud worker image (`publish-worker.yml`) no longer auto-publishes — focus shifted to local-git mode. This guide stays available for reference and reactivation.

 ...
 * at runtime — no hardcoded model lists. Used by Start Sprint provider
 * picker and any other surface that needs a live model list.
 *
 * Was previously at /api/wizard/models — relocated when the Wizard was
 * removed. The shape stays identical so existing callers only need a
 * path update.
 *
 * Returns:

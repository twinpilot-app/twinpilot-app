...
import { PipelineCreateSchema } from "@/lib/api-schemas";
...
export async function GET(req: NextRequest) {
  try {
    ...
    //    project_id (mig 191) included so Studio can split factory-scope
    //    vs project-scope pipelines on the same scope picker as agents.
    const { data: customs, error: cErr } = await sb
      .from("pipelines")
      .select("id, slug, name, description, type, category, plan_required, steps, is_active, created_at, factory_id, tenant_id, project_id, intent, origin_id")
      .eq("tenant_id", tenantId)
      .eq("is_active", true)
      .order("created_at");
...
export async function POST(req: NextRequest) {
  try {
    const { user, sb } = await getOperatorUser(req);
    // Validated body — Zod enforces tenantId/factoryId uuid, slug/name/
    // description caps, intent enum, AND each step's shape (step int>=1,
    // agent non-empty, gate "human"|null, phase int>=1|null,
    // phaseName non-empty). Mirrors the mig 185 DB CHECK.

...
export async function GET(req: NextRequest) {
  ...
  if (type === "contract") {
    ...
    // Canonical fallback (CLAUDE.md inviolable rule #7): agents under the
    // built-in tenant are referenced (not cloned) by every tenant. The
    // contract view must read both the caller's tenant AND built-in,
    // preferring the caller's row when the same slug exists in both.
    // Without this, viewing a SIPOC contract for a canonical agent
    // returns 404 whenever tenantId is sent. projectId, when provided,
    // takes priority above tenant-scope (mig 191 project-scoped agents).
    const sb = serviceClient();
    const tenantId  = req.nextUrl.searchParams.get("tenantId");
    const projectId = req.nextUrl.searchParams.get("projectId");

    const { data: builtInTenant } = await sb
      .from("tenants").select("id").eq("slug", "built-in").maybeSingle();
    const builtInTenantId = (builtInTenant?.id as string | undefined) ?? null;

    const tenantsToSearch = tenantId
      ? (builtInTenantId && builtInTenantId !== tenantId
          ? [tenantId, builtInTenantId]
          : [tenantId])
      : (builtInTenantId ? [builtInTenantId] : []);
    ...
    const projectRows: AgentRow[] = projectId
      ? (((await sb
          .from("agent_definitions")
          .select("name, spec, metadata, tenant_id, project_id")
          .eq("project_id", projectId)
          .eq("slug", agent)).data) ?? []) as AgentRow[]
      : [];

    const factoryRows: AgentRow[] = tenantsToSearch.length
      ? (((await sb
          .from("agent_definitions")
          .select("name, spec, metadata, tenant_id, project_id")
          .is("project_id", null)
          .eq("slug", agent)
          .in("tenant_id", tenantsToSearch)).data) ?? []) as AgentRow[]
      : [];

    const candidates = [...projectRows, ...factoryRows];
    // Priority: project-scope > tenant-scope > built-in canonical.
    const rank = (r: AgentRow): number =>
      projectId && r.project_id === projectId ? 0
      : r.project_id === null && tenantId && r.tenant_id === tenantId ? 1
      : 2;
    const agentDef = candidates.length === 0 ? null
      : candidates.reduce((best, r) => rank(r) < rank(best) ? r : best);

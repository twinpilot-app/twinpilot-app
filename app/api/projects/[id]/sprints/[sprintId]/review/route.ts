...
export async function GET(
  ...
  try {
    ...
    if (!factory) throw new NotFoundError(`Factory ${project.factory_id} not found`);

    const { data: member } = await sb
      .from("tenant_members").select("role")
      .eq("tenant_id", factory.tenant_id).eq("user_id", user.id).maybeSingle();
    if (!member) throw new ForbiddenError(`Caller is not a member of tenant ${factory.tenant_id}`);

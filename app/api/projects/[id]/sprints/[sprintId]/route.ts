...
import { ProjectSprintFinalizeSchema } from "@/lib/api-schemas";
...
export async function PATCH(
  ...
  try {
    const body = await parseBody(req, ProjectSprintFinalizeSchema);
    ...
    if (!TERMINAL_STATUSES.includes(targetStatus)) {
      throw new ValidationError(`status must be one of ${TERMINAL_STATUSES.join(" | ")}`, []);
    }
...
export async function DELETE(
  ...
  try {
    ...
    const { data: member } = await sb.from("tenant_members").select("role").eq("tenant_id", factory.tenant_id).eq("user_id", user.id).maybeSingle();
    ...
    if (ACTIVE_STATUSES.includes(sprint.status as string)) {
      return NextResponse.json({ error: `Cannot delete a sprint that is ${sprint.status as string}.`, code: "CONFLICT" }, { status: 409 });
    }

...
import { SprintSaveSchema } from "@/lib/api-schemas";
...
/**
 * Local serviceClient kept for downstream worker-mint paths in this file
 * that need an explicit instance separate from the auth gate. Auth flow
 * itself uses `getOperatorUser` from api-helpers.
 */
...
export async function POST(
  ...
  try {
    ...
    // Validated body — Zod enforces the action enum, targets/destinations
    // arrays (≤20 entries, each ≤64 chars). The action enum mirrors what
    // the route's downstream switch handles.
    const body = await parseBody(req, SprintSaveSchema);

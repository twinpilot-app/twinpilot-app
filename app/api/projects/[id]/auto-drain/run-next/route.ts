 ...
 * the same `attemptDrainOne` the CLI tick uses, so eligibility rules
 ...
 *
 * Returns a `DrainAttemptResult`. The browser surface (kanban "Run Next"
 * button) calls this for one-shot draining; for continuous draining
 * operators run `tp autodrain` which hits /api/cli/autodrain/tick.
 *
 * Historical note: a third consumer existed — the platform-side cron
 * `/api/cron/backlog-auto-drain`. Retired on 2026-05-04 (see memory
 * `feedback_no_platform_scheduling.md`).
 */

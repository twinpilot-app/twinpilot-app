...
import { getUser, parseBody, errorResponse } from "@/lib/api-helpers";
import { ProjectRunSchema, type ProjectRun } from "@/lib/api-schemas";
...
export async function POST(
  ...
  try {
    ...
    // Validated body — Zod enforces per-field caps (briefing 16KB, runNote
    // 2KB, agentInstruction 8KB, plus the global 1MB body cap). See
    // lib/api-schemas.ts for the contract.
    let body: ProjectRun = await parseBody(req, ProjectRunSchema);
    ...
    if (body.intent) {
      ...
      const { data: nextTodo } = await sb
        ...
        .limit(1)
        .maybeSingle();
      // Active items count — exclude done / cancelled. Those are
      // "post-work" archive states; their presence shouldn't pin the
      // heuristic to planning(grooming) forever once the kanban is
      // exhausted. Only items that still need attention (todo / doing)
      // count toward the grooming branch.
    ...
    const dispatch = await dispatchSprint({
      ...
      payload: {
        ...
        ...(agentInstructions ? { agentInstructions } : {}),
        ...(body.runMode === "pack-only" ? { runMode: "pack-only" } : {}),

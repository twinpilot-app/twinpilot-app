...
/**
 * Project-scoped membership gate. Looks up the project + tenant in the
 * same shot (also returns the project row so the caller can reuse the
 * prd_md field without a second query). Throws structured errors.
 */
async function assertProjectMember(req: NextRequest, projectId: string) {
  ...
  if (!member || !["platform_admin", "admin", "member"].includes(member.role as string)) {
    throw new ForbiddenError(`Caller lacks tenant membership on ${tenantId}`);
  }

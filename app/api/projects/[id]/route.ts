...
import {
  getUser, assertMember as assertMemberHelper, parseBody, errorResponse,
  type Infer,
...
/**
 * Local membership wrapper — keeps the existing throw-string-error contract
 * the rest of this route (DELETE handler) relies on. New code should prefer
 * `assertMemberHelper` from lib/api-helpers, which throws structured errors
 * the standard `errorResponse` translates.
 */
 ...
 *
 * Failure mode: when Trigger.dev rejects the cancel (auth bad, run already
 * gone, network glitch, API rate-limit), the worker keeps burning model
 * tokens for the in-flight agent even though the DB now says "paused".
 * Earlier this function returned silently on every error class — operators
 * had no surface to see "your pause didn't actually pause anything." We now
 * stamp the failure on `sprints.outcome.cancel_error` so the dashboard can
 * surface it.
 */
async function cancelActiveTriggerRun(
  ...
  try {
    ...
    resolvedSprintId = (sprint?.id as string | undefined) ?? null;
    if (!runId) return;
    ...
    // Capture the response so we can detect failures (404=already gone, 401/403
    // =auth issue, 429=rate limit, 5xx=trigger.dev outage) instead of trusting
    // the request reached the worker.
    const res = await fetch(`${TRIGGER_API}/api/v1/runs/${runId}/cancel`, {
      method: "POST",
      headers: { Authorization: `Bearer ${triggerKey}` },
    });

    if (!res.ok) {
      // 404 means the run already finished — that's fine, our DB status
      // update will close the loop. Anything else means the worker may
      // still be live and we need to surface it.
      const benign = res.status === 404;
      if (!benign && resolvedSprintId) {
        let detail = `Trigger.dev /cancel returned ${res.status}`;
        try {
          const body = await res.text();
          if (body) detail += `: ${body.slice(0, 200)}`;
        } catch { /* response body read failed — keep the status code */ }
        await sb
          .from("sprints")
          .update({
            status: sprintStatus,
            ...(sprintStatus !== "paused" ? { completed_at: new Date().toISOString() } : {}),
            outcome: {
              cancel_error: detail,
              cancel_attempted_at: new Date().toISOString(),
            },
          })
          .eq("id", resolvedSprintId);
        // Also surface to projects.last_error so the operator sees it on the
        // project card without drilling into sprint outcome JSON.
        await sb
          .from("projects")
          .update({ last_error: `Pause/cancel could not stop the worker on Trigger.dev (${res.status}). The dashboard says ${sprintStatus} but the run may still be consuming tokens. Cancel manually at https://cloud.trigger.dev/runs/${runId}.` })
          .eq("id", projectId);
        return;
      }
    }
    ...
    // Network failure / undefined fetch error / DB error — same surface
    // treatment so the operator sees something. Don't let pause silently lie.
    ...
    if (resolvedSprintId) {
      await sb
        ...
        .update({
          status: sprintStatus,
          ...(sprintStatus !== "paused" ? { completed_at: new Date().toISOString() } : {}),
          outcome: { cancel_error: `Cancel call threw: ${message}`, cancel_attempted_at: new Date().toISOString() },
        })
        .eq("id", resolvedSprintId)
        .then(() => null, () => null);
    }
    await sb
      .from("projects")
      .update({ last_error: `Pause/cancel failed before reaching Trigger.dev: ${message}. Sprint marked ${sprintStatus} in DB but the worker state is unknown.` })
      .eq("id", projectId)
      .then(() => null, () => null);
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    // Validated body — Zod enforces text-field caps (briefing 16KB,
    // PRD 64KB) and rejects unknown fields. The route below still
    // reads optional keys — Zod just guarantees the shapes are sane.
    const body: Infer<typeof ProjectPatchSchema> = await parseBody(req, ProjectPatchSchema);

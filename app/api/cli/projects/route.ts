...
import { CliProjectCreateSchema } from "@/lib/api-schemas";
...
export async function POST(req: NextRequest) {
  try {
    const body = await parseBody(req, CliProjectCreateSchema);
    ...
    if (existing) {
      return NextResponse.json(
        { error: `Project "${slug}" already exists (status: ${existing.status as string})`, code: "CONFLICT" },
        { status: 409 },
      );
    }
    ...
    if (dbSteps.length === 0) {
      throw new ValidationError(`Pipeline "${pipelineSlug}" not found or empty`, []);
    }
    ...
    const { data: created, error: createErr } = await auth.sb
      ...
      .insert({
        ...
        // acquires the slot and flips to 'running'.
        status: "queued",

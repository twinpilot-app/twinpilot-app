 ...
 *
 * Sole entrypoint for auto-drain since 2026-05-04. The platform-side
 * cron (`/api/cron/backlog-auto-drain` + `.github/workflows/auto-drain-cron.yml`)
 * was retired with the focus shift to local+git: scheduling tenant
 * tasks belongs in the tenant's own runtime, never in command-center
 * (see memory `feedback_no_platform_scheduling.md`).
 *
 * Authentication: tenant API key. Only touches projects of THAT tenant.
 * Cloud-mode projects work too — operators can run `tp autodrain` on a
 * VM/container if they want continuous draining for cloud sprints.
 *
 * Per-project work is the same `attemptDrainOne` helper that the
 * per-project "Run next" button uses, so eligibility rules (pause flag,
 * cooldown, daily cap, active window, unproductive guard, pending-save
 * proceed) stay consistent.
 *

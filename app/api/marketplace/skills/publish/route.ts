...
import { MarketplaceSkillsPublishSchema } from "@/lib/api-schemas";
...
async function getUserAndSkill(req: NextRequest, skillId: string) {
  ...
  if (!factory) throw new NotFoundError(`Factory ${skill.factory_id} not found`);
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Validated body — Zod enforces skill_id uuid, name 1-200,
    // description ≤8000, price_cents non-negative integer.
    const body = await parseBody(req, MarketplaceSkillsPublishSchema);
    const { sb, skill, factory } = await getUserAndSkill(req, body.skill_id);

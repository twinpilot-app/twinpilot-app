...
import { MarketplaceUninstallSchema } from "@/lib/api-schemas";
...
async function assertTenantAdmin(sb: SupabaseClient, userId: string) {
  const { data: member } = await sb
    .from("tenant_members").select("tenant_id, role")
    .eq("user_id", userId).maybeSingle();
  if (!member) throw new ForbiddenError("Caller has no tenant membership");
  if (!["platform_admin", "admin"].includes(member.role as string)) {
    throw new ForbiddenError("Caller lacks admin role");
  }
  return { tenantId: member.tenant_id as string };
}

export async function POST(req: NextRequest) {
  try {
    const body = await parseBody(req, MarketplaceUninstallSchema);
    ...
    if (body.kind === "pipeline") {
      const { tenantId } = await assertTenantAdmin(sb, user.id);
    ...
    if (!agentSlug || !targetFactoryId) {
      throw new ValidationError("agentSlug and targetFactoryId are required for legacy clone uninstall", []);
    }

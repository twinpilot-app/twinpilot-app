...
import {
  getOperatorUser, parseBody, errorResponse, serviceClient, NotFoundError,
} from "@/lib/api-helpers";
import { MarketplaceInstallSchema } from "@/lib/api-schemas";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: member } = await sb
      ...
      .eq("user_id", user.id)
      .single();
    if (!member) throw new NotFoundError("No tenant for caller");

    const tenantId = member.tenant_id as string;
    // Validated body — Zod enforces uuid shapes for listingId / target*
    // and the onConflict / mode enums (mig 171 install vs clone).
    const body = await parseBody(req, MarketplaceInstallSchema);

...
import { MarketplaceListingPatchSchema } from "@/lib/api-schemas";
...
async function getCallerTenantIds(sb: SupabaseClient, userId: string): Promise<string[]> {
...
export async function GET(
  ...
  try {
    ...
    if (!listing.factory_repo_id && !sourceFactoryId) {
      throw new ValidationError("Listing is not backed by a repo or a source factory", []);
    }
    ...
    if (!repoOwner || !repoName || !repoBranch || !factorySlug) {
      throw new ValidationError("Listing metadata incomplete", []);
    }
...
export async function PATCH(
  ...
  try {
    const body = await parseBody(req, MarketplaceListingPatchSchema);
    const { user, sb } = await getOperatorUser(req);
    const { id } = await params;
    ...
    if (!member || !["platform_admin", "admin"].includes(member.role as string)) {
      throw new ForbiddenError("Caller is not an admin of the publisher tenant");
    }

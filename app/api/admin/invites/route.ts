...
import { AdminInviteCreateSchema } from "@/lib/api-schemas";
...
export async function POST(req: NextRequest) {
  try {
    const body = await parseBody(req, AdminInviteCreateSchema);
    ...
    if (slug) {
      ...
      if (existing) {
        ...
        if (role !== "admin") {
          throw new ValidationError("Role must be admin when creating a new org", []);
        }

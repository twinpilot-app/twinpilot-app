...
import {
  ...
  serviceClient, ForbiddenError,
} from "@/lib/api-helpers";
import { AdminTenantPatchSchema } from "@/lib/api-schemas";

export const dynamic = "force-dynamic";

/** Admin gate — caller must have `app_metadata.role === 'admin'`. */
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    await assertAdmin(req);
    const sb = serviceClient();
    // Validated body — Zod enforces plan string cap, suspended boolean,
    // and the resetPassword sub-object (uuid + 8≤password≤128).
    const body = await parseBody(req, AdminTenantPatchSchema);
    const { id } = await params;
    ...
    if (body.resetPassword) {
      ...
      if (!membership) {
        return NextResponse.json({ error: "User not found in this tenant", code: "NOT_FOUND" }, { status: 404 });
      }

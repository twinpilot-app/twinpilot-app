...
import {
  errorResponse, parseBody, ValidationError, PayloadTooLargeError,
} from "@/lib/api-helpers";
import { AdminMaintenanceSchema } from "@/lib/api-schemas";
...
export async function POST(req: NextRequest) {
  const userId = await getOwnerUserId(req);
  if (!userId) return NextResponse.json({ error: "Forbidden", code: "FORBIDDEN" }, { status: 403 });

  let body: { action: "enable" | "disable" };
  try {
    body = await parseBody(req, AdminMaintenanceSchema);
  } catch (e) {
    if (e instanceof ValidationError || e instanceof PayloadTooLargeError) return errorResponse(e);
    throw e;
  }
  ...
  // Zod's enum check above ensures we never reach here.

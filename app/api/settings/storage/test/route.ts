...
import { SettingsStorageTestSchema } from "@/lib/api-schemas";
...
async function getTenantId(sb: SupabaseClient, userId: string): Promise<string> {
  const { data } = await sb
    ...
    .limit(1)
    .maybeSingle();
  if (!data) throw new NotFoundError("No tenant found");
...
export async function POST(req: NextRequest) {
  try {
    const body = await parseBody(req, SettingsStorageTestSchema);
    ...
    if (body.type) {
      ...
      if (error || !data) throw new NotFoundError(`Backend '${name}' not found`);

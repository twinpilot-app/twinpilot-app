...
import { SettingsIntegrationsGithubTestSchema } from "@/lib/api-schemas";
...
export async function POST(req: NextRequest) {
  try {
  const { tenantId } = await parseBody(req, SettingsIntegrationsGithubTestSchema);
  const { user, sb } = await getOperatorUser(req);

  // Membership check — operator must be in this tenant.

...
import { SettingsIntegrationsTestSchema } from "@/lib/api-schemas";

export const dynamic = "force-dynamic";

interface Step { name: string; ok: boolean; detail: string }

// ── Provider registry (same as /api/llm/models) ──────────────────────────────
...
export async function POST(req: NextRequest) {
  try {
    const { tenantId, serviceId } = await parseBody(req, SettingsIntegrationsTestSchema);
    const { user, sb: client } = await getOperatorUser(req);

...
import { serviceClient, parseBody, errorResponse } from "@/lib/api-helpers";
import { OnboardCreateTenantSchema } from "@/lib/api-schemas";
...
export async function POST(req: NextRequest) {
  try {
    // Validated body — Zod enforces email format + 8≤password≤128.
    // Tenant slug is normalised below (lowercase, regex-safe). The
    // route still validates against the invite's intent downstream.
    const body = await parseBody(req, OnboardCreateTenantSchema);
    const slug = normalizeSlug(body.tenantSlug);
    if (!slug) {
      return NextResponse.json({ error: "Tenant slug is required (after normalisation)", code: "VALIDATION_ERROR" }, { status: 400 });
    }

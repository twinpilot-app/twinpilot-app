...
/* ── Inline extension row (rendered inside a factory card) ── */
function ExtensionRow({
  ...
  ext: FactoryInfo;
  onToggle: (ext: FactoryInfo) => void;
  onDelete: (ext: FactoryInfo) => void;
  canDelete: boolean;
}) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 0", borderBottom: "1px solid var(--surface0)" }}>
      ...
      <button onClick={() => onToggle(ext)} style={{ background: "none", border: "none", cursor: "pointer", color: ext.enabled ? "var(--green)" : "var(--overlay0)", padding: 2, display: "flex" }}>
        {ext.enabled ? <ToggleRight size={15} /> : <ToggleLeft size={15} />}
      </button>
      {canDelete && (
        <button onClick={() => onDelete(ext)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--red)", padding: 2, display: "flex", opacity: 0.5 }}>
          <Trash2 size={12} />
        </button>
      )}
    </div>

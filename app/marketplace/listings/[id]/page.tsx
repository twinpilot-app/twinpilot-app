...
export default function MarketplaceListingDetailPage() {
  ...
  /**
   * Install OR update an agent. /api/marketplace/import refreshes the row in
   * place when the same listing is re-imported (origin_id match — YAML refresh
   * from HEAD); otherwise INSERTs a new row. After mig 188, two same-slug
   * agents from different publishers coexist in the same factory.
   */
  ...
  async function bulkReinstall() {
    ...
    if (selectedInstalled > 0) {
      const msg = selectedInstalled === selectedAgents.length
        ...
        : `Install ${selectedUninstalled} new agent(s) and reinstall ${selectedInstalled} existing one(s)?`;

...
export function CommandsSection({ factoryId, canWrite, projectId, selectMode }: {
  factoryId: string;
  canWrite:  boolean;
  projectId?: string;
  ...
  const [inheritedRows,  setInheritedRows]  = useState<CommandRow[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    // Main list — scope-dependent: factory rows in factory context,
    // project rows (the disable markers) in project context.
    let mainQ = supabase
    ...
    // Project context also fetches the inherited factory-defaults so
    // the picker can render them with checkbox state.
    if (projectId) {
      ...
      setInheritedRows((inh ?? []) as CommandRow[]);
    } else {
      setInheritedRows([]);
    }
  ...
  /** Disable an inherited factory-default command for this project. */
  async function disableInherited(inh: CommandRow) {
    if (!projectId) return;
    const { error } = await supabase.from("factory_slash_commands").insert({
  ...
  async function reEnableInherited(inh: CommandRow) {
    const override = rows.find((r) => r.slug === inh.slug);
    if (!override) return;
    const { error } = await supabase.from("factory_slash_commands").delete().eq("id", override.id);
    if (error) { setError(error.message); return; }
    void load();
  }
  ...
  // No authoring chrome. Operator picks which factory commands apply
    ...
    return (
      <div>
        <p style={{ fontSize: 11, color: "var(--overlay0)", margin: "0 0 10px", lineHeight: 1.55 }}>
          Pick which factory-default commands apply to this project. Authoring lives in Studio &gt; Commands.
        </p>
        {error && <div style={errBanner}>{error}</div>}
        {loading ? (
          <div style={muted}>Loading…</div>
        ) : inheritedRows.length === 0 ? (
          <div style={muted}>No factory commands available yet. Add some in Studio &gt; Commands first.</div>
        ) : (
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                      ...
                      <code style={{ fontSize: 11, color: "var(--blue)", fontFamily: "var(--font-mono)" }}>/{inh.slug}</code>
                    </div>
                    {inh.description && (
      ...
      <StudioSectionHeader
        ...
        actions={canWrite && (
          <>
            ...
            <button onClick={() => setShowGhImport(true)} style={studioBtnGhost}>Import from GitHub</button>
            <button onClick={() => setShowCurated(true)}  style={studioBtnGhost}>Curated Index</button>
          </>
        )}
      />
...
function CuratedIndexModal({ factoryId, onClose, onSaved }: {
  ...
  return (
    <CuratedIndexModalShell
      title="Curated index — slash commands"
      icon={<ListChecks size={14} />}
      curatedKind="commands"
      pathKey="commands"
      onLoad={loadIndex}
      onAction={selected.size > 0 ? batchInstall : undefined}
      actionLabel={selected.size > 0 ? `Import ${selected.size}` : undefined}

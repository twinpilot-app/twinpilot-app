...
/** Operator-facing project configuration dialog. Owns its own useStates
 *  and bubbles `onSaved(updated)` up to the host page. */
import React, { useEffect, useMemo, useState } from "react";
import {
  AlertTriangle, Bot, Brain, CheckCircle2, ChevronDown, ExternalLink,
  FileText, GitBranch, Layers, ListTodo, Package, RefreshCw, Save,
  Search, Settings, Sparkles, Terminal, Trash2, X, Zap,
} from "lucide-react";
import type { Project } from "@/components/ProjectCard";
import { SkillsSection } from "@/components/SkillsSection";
import { CommandsSection } from "@/components/CommandsSection";
import { HooksSection } from "@/components/HooksSection";
import MarkdownPreviewButton from "@/components/MarkdownPreviewButton";
 ...
 */
const GUIDED_BRIEFING_TEMPLATE = `# What — in one sentence, what is this?
<describe the product / feature in 1-2 sentences>

# Who — who uses it, and when?
<target persona, usage context>

# Why — what problem does it solve, or what opportunity does it open?
<current pain, or gain unlocked>

# How we'd know it's working — observable metric or signal (optional)
<success metric, exit criterion, qualitative signal>
`;
...
function ProjectSettingsModal({ project, pipelines, onClose, onSaved, inline }: {
  ...
  const [collapsed, setCollapsed] = useState<Record<string, boolean>>({
    briefing: false,                                                              // core surface — open by default
    prd: true,                                                                    // optional secondary — collapsed by default
  ...
  // Platform Agents (mig 190) — per-project picker. Defaults shipped in
  // mig 190 are { enabled:true, mode:"cli", cli:"claude-code",
  // model:"claude-haiku-4-5" }. Operator overrides via this section;
  // dispatchPlatformAgent reads projects.settings.platform_agents.
  const platformAgentsCfg = (s as { platform_agents?: { enabled?: boolean; mode?: "cli" | "api"; cli?: string; model?: string } }).platform_agents ?? {};
  const [paEnabled, setPaEnabled] = useState<boolean>(platformAgentsCfg.enabled ?? true);
  const [paMode,    setPaMode]    = useState<"cli" | "api">(platformAgentsCfg.mode ?? "cli");
  const [paCli,     setPaCli]     = useState<string>(platformAgentsCfg.cli ?? "claude-code");
  const [paModel,   setPaModel]   = useState<string>(platformAgentsCfg.model ?? "claude-haiku-4-5");
  ...
  const { session: authSession, tenantId, factoryId: ctxFactoryId, factorySlug: ctxFactorySlug, factoryName: ctxFactoryName } = useAuth();
  // Tenant slug — used to render the resolved local agent workdir hint.
  // Not exposed by useAuth; one-shot fetch keyed off tenantId.
  const [tenantSlug, setTenantSlug] = useState<string>("");
  useEffect(() => {
    if (!tenantId) { setTenantSlug(""); return; }
    let cancelled = false;
    void supabase.from("tenants").select("slug").eq("id", tenantId).maybeSingle()
      .then(({ data }) => { if (!cancelled && data?.slug) setTenantSlug(data.slug as string); });
  ...
  useEffect(() => {
    ...
    fetch("/api/llm/models", { headers: { Authorization: `Bearer ${authSession.access_token}` } })
      .then(async (res) => {
        if (res.ok) {
  ...
  /** Brief "Saved." flash so the operator sees confirmation when the
   *  Save button no longer closes the modal (only the X icon does). */
  ...
  async function handleSave() {
    ...
    const settings: ProjectSettings = {
      ...
      // Platform Agents per-project config (mig 190). Defaults exist in
      // the worker (dispatchPlatformAgent) — only persist what the
      // operator explicitly set so future default changes propagate.
      platform_agents: {
        enabled: paEnabled,
        mode:    paMode,
        cli:     paCli,
    ...
    // Save no longer closes the modal — operator dismisses via the X
    // icon on the header. Flash a brief confirmation so the click
    // doesn't feel inert.
    setSavedFlash(true);
    ...
    <div style={inline
      ...
      <div style={inline ? {} : { background: "var(--mantle)", border: "1px solid var(--surface0)", borderRadius: 18, width: "min(720px, 95vw)", maxHeight: "90vh", overflowY: "auto", boxShadow: "0 24px 64px rgba(0,0,0,0.5)" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 20px", borderBottom: "1px solid var(--surface0)", position: "sticky", top: 0, background: "var(--mantle)", zIndex: 1 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <button
              onClick={handleSave}
              disabled={saving}
              title="Save changes — modal stays open. Use the X to dismiss."
              style={{
                display: "flex", alignItems: "center", gap: 5,
                padding: "6px 14px", borderRadius: 8, border: "none",
                background: savedFlash ? "var(--green)" : "var(--blue)",
            ...
            <button onClick={onClose} title="Close (Esc)" style={{ background: "transparent", border: "none", cursor: "pointer", color: "var(--overlay0)", padding: 4 }}><X size={18} /></button>
          </div>
        </div>
        ...
        <div style={{ padding: "18px 20px" }}>
          ...
          {/* ── Workspace Pack ─────────────────────────────────────────
             Project Pack: every agent across all pipelines + skills,
             commands, hooks, output-styles, permissions and the .tp/*
             context. Materialises and cleans up the full project
             scaffold without dispatching a sprint. The Start Sprint
             modal hosts the per-sprint counterpart (Sprint Pack), which
             scopes agents to the dispatched sprint's intent pipeline. */}
          {(orchMode === "local" || orchMode === "local-git") && (
            <div style={{ marginBottom: 18 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                <Package size={14} color="var(--blue)" />
                <span style={{ fontSize: 13, fontWeight: 700, color: "var(--text)" }}>Workspace Pack</span>
                <span style={{ fontSize: 10, color: "var(--overlay0)" }}>
                  Install / remove the full {brand.shortName} project scaffold on your local workdir. Sprint-scoped install lives in the Start Sprint modal.
                </span>
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 8, padding: "12px 14px", borderRadius: 8, background: "var(--crust)", border: "1px solid var(--surface0)" }}>
                <ProjectPackControls
          ...
          <SectionHeader
            id="briefing"
            title="Briefing"
            icon={<FileText size={14} color="var(--blue)" />}
            collapsed={collapsed.briefing ?? false}
            onToggle={() => toggleSection("briefing")}
            // eslint-disable-next-line react/no-unstable-nested-components -- IIFE returning JSX, not a component definition; lint rule false-positive
            badge={(() => {
              const len = briefing.trim().length;
              if (len === 0) return <span style={{ fontSize: 10, color: "var(--peach)" }}>empty</span>;
              if (len < 80)  return <span style={{ fontSize: 10, color: "var(--peach)" }}>{len} chars (need ≥80)</span>;
              return <span style={{ fontSize: 10, color: "var(--green)" }}>{len} chars</span>;
            })()}
          />
          {!collapsed.briefing && (
            <div style={{ marginBottom: 14 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                <span style={{ flex: 1, fontSize: 10, color: "var(--overlay0)", lineHeight: 1.5 }}>
                  Markdown. Read by every agent at sprint start as the tagline / problem statement.
                </span>
                <MarkdownPreviewButton
                  value={briefing}
                  label="Briefing"
                  hint="Rendered preview of the markdown above. Source of truth stays the textarea."
                />
                <button
                  type="button"
                  onClick={() => {
              ...
              />
              {/* Drift warning: briefing was edited AND there are
                  uncompleted backlog items. Those items were authored
                  under the OLD briefing — the system won't re-evaluate
                  them. Operator owns coherence; this is the visible
                  reminder. */}
          ...
          <SectionHeader
            ...
            icon={<FileText size={14} color="var(--mauve)" />}
            collapsed={collapsed.prd ?? true}
            onToggle={() => toggleSection("prd")}
            // eslint-disable-next-line react/no-unstable-nested-components -- IIFE returning JSX, not a component definition; lint rule false-positive
            badge={(() => {
              const len = prdMd.trim().length;
              if (len === 0) return <span style={{ fontSize: 10, color: "var(--overlay0)" }}>empty</span>;
              return <span style={{ fontSize: 10, color: "var(--green)" }}>{len.toLocaleString()} chars</span>;
            })()}
          />
          {!collapsed.prd && (
            <div style={{ marginBottom: 14 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                <span style={{ flex: 1, fontSize: 10, color: "var(--overlay0)", lineHeight: 1.5 }}>
                  Optional structured markdown. Used by the kanban&apos;s <strong>Generate from PRD</strong> button to extract backlog items via LLM. Briefing stays the agent-facing tagline; PRD is the deeper spec.
                </span>
                <MarkdownPreviewButton
                  value={prdMd}
                  label="PRD"
                  hint="Rendered preview of the PRD markdown."
                />
              </div>
              <textarea
                value={prdMd}
          ...
          <SectionHeader
            ...
            icon={<Bot size={14} color="var(--mauve)" />}
            collapsed={collapsed["platform-agents"] ?? true}
            onToggle={() => toggleSection("platform-agents")}
            badge={<span style={{ fontSize: 10, color: paEnabled ? "var(--green)" : "var(--overlay0)", fontWeight: 600 }}>
              {paEnabled ? `${paMode === "cli" ? paCli : paMode} / ${paModel}` : "off"}
            </span>}
          />
          {!collapsed["platform-agents"] && (
            <div style={{ marginBottom: 14, display: "flex", flexDirection: "column", gap: 12 }}>
              <div style={{ fontSize: 11, color: "var(--overlay0)", lineHeight: 1.55 }}>
                Cheap LLM calls that smooth your workflow — generating commit titles, sprint narratives, briefing validations, failure translations. They run in fixed hooks of the sprint, never appear in your pipeline, and silently fall back to deterministic heuristics on failure. Catalog managed in <code style={{ fontFamily: "var(--font-mono)" }}>/admin/platform-agents</code>.
              </div>
              <label style={{ display: "flex", alignItems: "flex-start", gap: 9, padding: "10px 12px", borderRadius: 8, background: "var(--surface0)", border: "1px solid var(--surface1)", cursor: "pointer" }}>
                <input type="checkbox" checked={paEnabled} onChange={(e) => setPaEnabled(e.target.checked)} style={{ marginTop: 2 }} />
                <div>
                  <div style={{ fontSize: 12, fontWeight: 600, color: "var(--text)" }}>Enable platform agents on this project</div>
                  <div style={{ fontSize: 10, color: "var(--overlay0)", marginTop: 2, lineHeight: 1.5 }}>
                    When off, the worker uses the deterministic fallback at every hook — sprint titles + summaries stay accurate but lose the narrative polish.
                  </div>
                </div>
              </label>
              {paEnabled && (
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                  <div>
                    <label style={{ display: "block", fontSize: 11, fontWeight: 600, color: "var(--subtext0)", marginBottom: 4 }}>Mode</label>
                    <select value={paMode} onChange={(e) => setPaMode(e.target.value as "cli" | "api")} style={ss}>
                      <option value="cli">CLI (claude-code)</option>
                      <option value="api">API (Anthropic)</option>
                    </select>
                    <div style={{ fontSize: 10, color: "var(--overlay0)", marginTop: 3, lineHeight: 1.4 }}>
                      {paMode === "cli"
                        ? "Spawns claude-code with no MCP, plan permission, 1-turn cap. Uses operator's local claude-code auth."
                        : "Direct Anthropic API call. Uses tenant's ANTHROPIC_API_KEY from Integrations."}
                    </div>
                  </div>
                  <div>
                    <label style={{ display: "block", fontSize: 11, fontWeight: 600, color: "var(--subtext0)", marginBottom: 4 }}>Model</label>
                    <select value={paModel} onChange={(e) => setPaModel(e.target.value)} style={ss}>
                      <option value="claude-haiku-4-5">claude-haiku-4-5 (default — cheapest)</option>
                      <option value="claude-sonnet-4-5">claude-sonnet-4-5 (richer; ~5x cost)</option>
                      <option value="claude-opus-4-7">claude-opus-4-7 (overkill — only for tricky personas)</option>
                    </select>
                    <div style={{ fontSize: 10, color: "var(--overlay0)", marginTop: 3, lineHeight: 1.4 }}>
                      Haiku is the recommended default — these tasks are tiny (≤200 tokens output). Bumping to sonnet only adds value for narrative-heavy hooks like sprint.summary.
                    </div>
                  </div>
                </div>
              )}
                ...
                <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 10 }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 8, padding: "10px 12px", borderRadius: 8, background: "var(--crust)", border: "1px solid var(--surface0)" }}>
                    ...
                    <div style={{ fontSize: 10, color: "var(--overlay0)", marginLeft: 98 }}>
                      Required for {orchMode}. Agent workdir = <code style={{ fontFamily: "monospace" }}>{(cliLocalBasePath || globalBasePath || "<base path>")}/TwinPilotProjects/{tenantSlug || "{org-slug}"}/{ctxFactorySlug || "{factory-slug}"}/{project.slug}</code>
                    </div>
                  </div>
            ...
            <>
              ...
              {!collapsed.cli && (
                <div style={{ paddingLeft: 8, display: "flex", flexDirection: "column", gap: 10, marginBottom: 14 }}>
                  ...
                  {/* Workspace Pack actions (Prepare workspace + Install/
                      Remove pack) live in the dedicated Workspace Pack
                      section near the top of this panel. CLI Agents only
                      owns CLI runtime config below. */}
              ...
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                  ...
                  <MarkdownPreviewButton
                    ...
                    hint="Rendered preview of the guidelines text injected into every agent's persona."
                  />
                </div>
          ...
          {/* ── 5b. Skills (project picker — selectMode) ─────────────
               Authoring lives in Studio. This panel just selects which
               factory-default skills/commands/hooks apply to the project. */}
          ...
          {/* ── 5c. Slash Commands (project picker) ───────────────── */}
          <SectionHeader id="commands" title="Slash Commands" icon={<Terminal size={14} color="var(--blue)" />}
            collapsed={collapsed.commands ?? false} onToggle={() => toggleSection("commands")}
          />
          {!collapsed.commands && (
            <div style={{ marginBottom: 14 }}>
              <CommandsSection
          ...
          {/* ── 5d. Hooks (project picker) ─────────────────────────── */}
          <SectionHeader id="hooks" title="Hooks" icon={<Zap size={14} color="var(--peach)" />}
            collapsed={collapsed.hooks ?? false} onToggle={() => toggleSection("hooks")}
          />
          {!collapsed.hooks && (
            <div style={{ marginBottom: 14 }}>
              <HooksSection
        ...
        {!inline && (
          <div style={{ padding: "12px 20px", borderTop: "1px solid var(--surface0)", display: "flex", gap: 10, position: "sticky", bottom: 0, background: "var(--mantle)" }}>
            <button onClick={onClose} style={{ padding: "9px 18px", borderRadius: 9, border: "1px solid var(--surface1)", background: "transparent", color: "var(--subtext0)", fontSize: 13, cursor: "pointer", fontFamily: "var(--font-sans)" }}>Close</button>
            <button
              onClick={handleSave}
              disabled={saving}
              style={{
                ...
                background: savedFlash ? "var(--green)" : "#1463ff",
...
/**
 * Project Pack controls — Install + Remove pair.
 *
 * Install dispatches the `prepare-workspace` Trigger task: materialises
 * the FULL project scaffold (CLAUDE.md, .claude/agents/* across every
 * pipeline, .mcp.json, .claude/skills, commands, hooks, output-styles,
 * permissions, .tp/* and the pack manifest) without dispatching any
 * sprint. Operator runs claude-code (or any MCP-capable CLI) themselves.
 *
 * Remove reads .tp/pack-manifest.json and deletes every file the
 * platform wrote — operator-authored files stay untouched. The manifest
 * format is identical between Project Pack and Sprint Pack, so Remove
 * Project Pack uses the same cleanup endpoint.
 *
...
function ProjectPackControls({
  ...
  const blocked = requiresRepoUrl || !authToken;
  const blockedReason =
  ...
  async function install() {
    if (blocked || busyKind) return;
    setBusyKind("install"); setMessage(null);
    try {
      ...
      if (!res.ok) setMessage({ kind: "err", text: body.error ?? "Install Project Pack failed." });
      else         setMessage({ kind: "ok",  text: body.message ?? "Project Pack install dispatched." });
    } catch (e) {
  ...
  async function remove() {
    if (blocked || busyKind) return;
    if (!confirm(`Remove the Project Pack from your local workspace? Only files ${brand.shortName} wrote (per the manifest) will be deleted; your other files stay.`)) return;
    setBusyKind("remove"); setMessage(null);
    try {
      const res = await fetch(`/api/projects/${projectId}/pack/remove`, {
        method:  "POST",
        headers: { Authorization: `Bearer ${authToken}`, "Content-Type": "application/json" },
        body:    JSON.stringify({}),
      });
      const body = await res.json().catch(() => ({})) as { error?: string };
      if (!res.ok) setMessage({ kind: "err", text: body.error ?? "Remove Project Pack failed." });
      else         setMessage({ kind: "ok",  text: "Project Pack remove dispatched. Workspace will be cleaned in seconds." });
    } catch (e) {
      setMessage({ kind: "err", text: (e as Error).message ?? "Network error." });
    } finally {
      setBusyKind(null);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
        <button
          onClick={install}
          disabled={blocked || busyKind !== null}
          title={blocked ? blockedReason : "Materialise the full project scaffold (every agent across all pipelines + skills, commands, hooks, output-styles, permissions, .tp/* and the pack manifest) at the project's local workdir."}
          style={{
            display: "flex", alignItems: "center", gap: 6,
            padding: "8px 12px", borderRadius: 8,
            border: "1px solid var(--surface1)",
            background: blocked || busyKind !== null ? "var(--surface0)" : "rgba(166,209,137,0.10)",
            color: blocked ? "var(--overlay0)" : "var(--green)",
            fontSize: 12, fontWeight: 600,
            cursor: blocked || busyKind !== null ? "not-allowed" : "pointer",
            opacity: blocked || busyKind !== null ? 0.6 : 1,
            fontFamily: "var(--font-sans)",
          }}
        >
          {busyKind === "install"
        ...
        <button
          onClick={remove}
          disabled={blocked || busyKind !== null}
          title={blocked ? blockedReason : `Read .tp/pack-manifest.json and delete every file ${brand.shortName} wrote.`}
          style={{
            display: "flex", alignItems: "center", gap: 6,
            padding: "8px 12px", borderRadius: 8,
            border: "1px solid rgba(228,75,95,0.3)",
            background: blocked || busyKind !== null ? "var(--surface0)" : "rgba(228,75,95,0.06)",
            color: blocked ? "var(--overlay0)" : "var(--red)",
            fontSize: 12, fontWeight: 600,
            cursor: blocked || busyKind !== null ? "not-allowed" : "pointer",
            opacity: blocked || busyKind !== null ? 0.6 : 1,
            fontFamily: "var(--font-sans)",

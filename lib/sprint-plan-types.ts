...
import type { SprintIntent, PlanningSubmode } from "./types";

/**
 * Snapshot of every per-intent pipeline slot the project has — assigned
 * AND unassigned. The Review modal renders this as a "manifest" so the
 * operator sees the full pipeline routing config at a glance, not just
 * the one slot that's resolved for the current sprint's intent.
 *
 * The `default` slot is the legacy single-pipeline (`projects.pipeline_id`)
 * — kept around as a fallback for projects that haven't migrated to the
 * per-intent model. New projects can leave it empty.
 */
export interface SprintPlanPipelineSlot {
  /** The pipelines.id this slot points at, or null when unassigned. */
  id:        string | null;
  /** Display name. Null when the slot is unassigned. */
  name:      string | null;
  /** Step count from the pipeline rows. 0 when unassigned. */
  stepCount: number;
  /** True when this slot's pipeline is what the current sprint actually
   *  loaded. Exactly one slot is active per plan. */
  active:    boolean;
}
...
export interface SprintPlan {
  ...
  project: {
    ...
    repoUrl?:     string;
    intakeBrief?: string;
    /** The single pipeline that's active for THIS sprint (resolved from
     *  the intent below). Kept for back-compat with v1 plans — new
     *  consumers should prefer `pipelineSlots` for full disclosure. */
    pipeline:     { slug: string; name: string; stepCount: number };
    /** Full per-intent pipeline manifest — every slot, assigned or not.
     *  The Review modal uses this so the operator sees the project's
     *  whole routing config, not just the active slot. */
    pipelineSlots: {
      discovery: SprintPlanPipelineSlot;
      planning:  SprintPlanPipelineSlot;
      execution: SprintPlanPipelineSlot;
      review:    SprintPlanPipelineSlot;
      default:   SprintPlanPipelineSlot;
    };
    /** When true, /run derives the sprint's intent from project state.
     *  When false, the operator picks the intent at Start Sprint. */
    heuristicIntent: boolean;
  ...
  sprint: {
    ...
    /** Sprint shape (discovery / planning / execution / review). Drives
     *  which pipeline slot is loaded above, and which agent input shape
     *  the worker constructs. Mig 117 + 169. */
    intent:           SprintIntent;
    /** When intent='planning', narrows what PO does. Surfaced as MCP
     *  signal at runtime; recorded here for plan auditability. */
  ...
  execution: {
    ...
    /**
     * Whether auto-push to the working repository runs at sprint end.
     * Reflects project.settings.cli_agents.git_auto_push (default true;
     * operator opts out via Storage Location toggle). Worker reads the same
     * shape via run-pipeline.ts (`!== false`).
     */
    gitAutoPush?:          boolean;
    /**
     * Resolved working repository for local-git mode. Source of truth:
     *   1. project.working_destination_id → factory_output_destinations.owner
     *   2. legacy project.repo_url
     * Undefined when neither is configured (post-flight push will skip).
     */
    workingRepository?: {
      url:    string;
      branch: string;

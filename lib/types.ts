...
export interface ProjectSettings {
  ...
  /**
   * Platform Agents per-project config (mig 190). Cheap LLM calls fired
   * in fixed hooks of the sprint flow (commit titles, sprint summaries,
   * briefing validation, …). Defaults applied by the worker when this
   * field is absent: enabled=true, mode='cli', cli='claude-code',
  ...
  platform_agents?: {
    ...
    cli?:     string;
    model?:   string;
    /** Per-hook overrides (rare). Keys match agent_definitions.platform_hook. */
    hooks?:   Record<string, Partial<{ mode: "cli" | "api"; cli: string; model: string }>>;
  };
  /**
...
/**
 * `Project` augmented with optional DB columns that Supabase queries return
 * but the shared `Project` type doesn't declare (sprint counters, intake brief,
 * last error, mode label). Used by the Office page and StartSprintModal where
 * we read those columns directly from the row.
 */

 ...
 * CLI tick endpoint (POST /api/cli/autodrain/tick — driven by `tp autodrain`)
 * and the per-project "Run next" endpoint (POST /api/projects/[id]/auto-drain/run-next).
 *
 * Both consumers do the SAME eligibility checks for a given project and
 * dispatch the next backlog item; the difference is the outer auth gate
 * (CLI tenant API key vs tenant member session) and the scan shape (CLI
 * tick iterates the tenant's projects, the per-project endpoint runs
 * against one).
 *
 ...
 * timer is enforced.
 *
 * Historical note: a third consumer existed — the platform-side cron
 * `/api/cron/backlog-auto-drain` driven by `.github/workflows/auto-drain-cron.yml`.
 * Both retired on 2026-05-04 (see memory `feedback_no_platform_scheduling.md`).
 */
